...
const { validateFirstStipendApplication } = require("../validation/StipendApplicationValidation");
...
exports.createFirstStipendApplication = catchAsyncError(async (req, res) => {
	...
	try {
		...
		try {
			...
			Logger.error(`Error creating user ${email} for first stipend application`, err);
		...
		if (newUser?._id) {
			...
			try {
				...
				Logger.error(`Error creating first stipend application for user ${email}`, err);
				return res.status(500).json({
					error: err
				});
			}
...
exports.verifyUser = catchAsyncError(async (_, res) => {
	...
	try {
		...
		const { token } = await Authentication.getTokenForAuthenticatedUser(username, user, TokenExpiration.TWO_WEEKS);
		res.status(201).json({
			success: true,
