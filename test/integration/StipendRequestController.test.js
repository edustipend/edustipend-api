...
const { goodRequestData, badRequestData, badRequestDataType } = require('../dummyData')
...
describe('Test for Stipend Request', function () {
  ...
  describe('Test for sending stipend request', function () {
    this.beforeAll(async function () {
      ...
      res = await chai.request(server).post('/v1/request-stipend').send(goodRequestData)
  ...
  describe('Stipend request with incomplete data should fail', function () {
    this.beforeAll(async function () {
      ...
      res = await chai.request(server).post('/v1/request-stipend').send(badRequestData)
  ...
  describe('Stipend request with wrong data types should fail', function () {
    this.beforeAll(async function () {
      ...
      res = await chai.request(server).post('/v1/request-stipend').send(badRequestDataType)
