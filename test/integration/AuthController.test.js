...
const { registerUser, badRegisterUserRequest,badResetRequest,passwordReset } = require("../dummyData");
...
describe("Test for AuthController", function () {
  ...
  describe("Test for rest password", async function (){
    this.beforeAll(async function (){
      this.timeout(0)
      res = await chai.request(server).post("/reset-password").send(passwordReset)
    })
    it("should be able to reset a user password", async function (){
      expect(res).to.have.status(201);
      expect(res.body.success).to.equal(true);
      expect(res.body.message).to.equal("Please check your email for a reset password code")
    })
   })
  
   describe("User cannot reset password with incomplete data", async function(){
    this.beforeAll(async function(){
  this.timeout(0)
    })
    res = await chai.request(server).post("v1/reset-password").send(badResetRequest)
   })
   it("should be able to succesfully reset password",async function(){
    expect(res).to.have.status(500);
    expect(res.body.success).to.equal(false)
   })
});
